...
def index() : 
    if 'logged_in' in session:
        ...
        return "please identify yourself: ../login (or register you : ./register) for on Postman !"
...
def artCrud(id=None):
    """
    1/ verify if the connected user is the owner of the articles. if yes, set 'owner' at true
    2/ if owner is true he can delete the article
    if not he can only read
    """
    owner = False
    ...
    if 'logged_in' in session:
        ...
        message="To access a specific article, you need to be authenticated"
        return message
